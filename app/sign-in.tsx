  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (result.error) {
        ...
        if (settings != null) {
          settings.vehicleNumber = email.replace(
            "@" + process.env.EXPO_PUBLIC_EMAIL_DOMAIN,
            "",
          );
        } else {
          settings = {
            vehicleNumber: email.replace(
              "@" + process.env.EXPO_PUBLIC_EMAIL_DOMAIN,
              "",
            ),
          };
  ...
  return (
    <ThemedView style={styles.globalContainer}>
      ...
      <ThemedText style={styles.loginText}>
        Введите логин и пароль или наведите телефон на QR код
      </ThemedText>
