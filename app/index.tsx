  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        try {
          const apps = await makeAuthenticatedRequest(
            "applications?userId=" + session?.user.id,
          );
        ...
        const _currentBus: any = await getBusByNumber(
          _db,
        ...
        if (_currentBus === null) {
          Alert.alert(
            "Автобус с номером " +
              settings.vehicleNumber +
              " не найден в вашей учетной записи. Зайдите другим пользователем.",
          );
        ...
        try {
          ...
          routes = (await getRoutes(_db)).map((item: any) =>
        ...
        if (timeSlots?.length > 0) {
          timeSlots.map((slot: any) => {
            if (
              new Date(slot.startTimestamp).getTime() < new Date().getTime() &&
              new Date().getTime() < new Date(slot.endTimestamp).getTime()
            ) {
        ...
        const currentRoute = routes.filter(
          (route: any) => route.id === _currentBus.routeId,
        );
        if (currentRoute.length === 0) {
          Alert.alert(
            "Для вашего ТС нет маршрута. Войдите другим пользователем.",
          );
          router.push("/settings");
          return;
        } else {
          ...
          getNewAccessCards(
            _db,
            session?.user.id,
            currentRoute[0].organization.id,
          );
        }
      } catch (error: any) {
