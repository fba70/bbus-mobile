...
class NfcProxy {
  ...
  readTag = withAndroidPrompt(async () => {
    ...
    try {
      ...
      tag = await NfcManager.getTag() as TagEvent & {
        ndefStatus: {
            status: NdefStatus;
            capacity: number;
        }
  ...
  writeNdef = withAndroidPrompt(async ({type, value}: {type: string, value: any}) => {
    let result = false;
  ...
  customTransceiveIsoDep = withAndroidPrompt(async (commands: any) => {
    let result = false;
    const responses = [];
